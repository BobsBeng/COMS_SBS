...
$(document).ready(function () {
    var temp = $('#rowNum').text();
    //console.log(temp);
    if (temp.trim() !== "") {
        counter = parseInt(temp, 10) + 1;
    } else {
        counter = 1;
    }
    //console.log(counter);
...
function addLine(url) {
    ...
    else if (addDays(Date.parse($('#orderDate').val()), lt) > Date.parse($('#deliveryDate').val())) {
        isAllValid = false;
...
function saveData(url, saveStatus, uTate, indexUrl) {
    ...
    // Get the T_SLO_DT value in yyyy/MM/dd format
    var tSLO_DT = $('#orderDate').val();
    // Split the date string into year, month, and day components
    var parts = tSLO_DT.split('/');
    var year = parts[0];
    var month = parts[1];
    var day = parts[2];
    ...
    for (let i = 0; i < getAll.length; i++) {
        var orderList = {
            ...
            RQST_ARVD_DT: $('#inputDetailsItems').DataTable().cell(i, 7).data().trim(),
            RQST_DLV_DT: $('#inputDetailsItems').DataTable().cell(i, 8).data().trim(),
    ...
    var sumOrder = {
        ...
        T_SLO_DT: $('#orderDate').val(),
        //T_SLO_DT:formattedDate,
